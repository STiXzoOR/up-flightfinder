	...
	$.fn.seatCharts = function (setup) {
		...
		$.each(settings.map, function(row, characters) {
			...
			if (settings.naming.left) {
				$row.append(
					$('<div></div>')
						.addClass('seatCharts-row-number')
			...
			$.each(characters.match(/[a-z_]{1}(\[[0-9a-z_]{0,}(,[0-9a-z_ ]+)?\])?/gi), function (column, characterParams) { 
				...
				$row.append(character != '_' ?
					...
					(function(naming) {
						...
						var label = overrideLabel ? overrideLabel : naming.getLabel(character, naming.rows[row], naming.columns[column]);
					...
					$('<div></div>').addClass('seatCharts-cell seatCharts-space').text(settings.naming.rows[row])
				);
			});
